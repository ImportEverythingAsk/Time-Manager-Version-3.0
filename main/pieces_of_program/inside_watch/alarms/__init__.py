...
import math
class All_Alarms:
    def __init__(self, watch_frame, total_alarms, watch_id):
    ...
    def render_alarms(self):
        ...
        for fullrow in range(0, floored):
            for column in range(0, 3):
                currentAlarm = Alarm(alarm_frames, fullrow, column, self.watch_id)
                currentAlarm.render()
        ...
        for alarm in range(0, remaining_number_of_alarms):
            currentAlarm = Alarm(alarm_frames, floored, alarm, self.watch_id)
            currentAlarm.render()
            # if self.this_is_a == "make":
            #     currentAlarm.make_data()
            # else:
