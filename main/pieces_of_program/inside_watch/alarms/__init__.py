...
class All_Alarms:
    ...
    def render_alarms(self):
        self.alarm_frames = Frame(self.watch_frame, bg=self.app.secondary_bg_color)
        self.alarm_frames.grid(row=1, column=0, pady=20, padx=20)
        ...
        for fullrow in range(0, floored):
            for column in range(0, 3):
                currentAlarm = Alarm(self.app, self.alarm_frames, fullrow, column, self.watch_id)
                currentAlarm.render()
        ...
        for alarm in range(0, remaining_number_of_alarms):
            currentAlarm = Alarm(self.app, self.alarm_frames, floored, alarm, self.watch_id)
            currentAlarm.render()
            # if self.this_is_a == "make":
            #     currentAlarm.make_data()
            # else:
